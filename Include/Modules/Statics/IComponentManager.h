...
public:
    ...
    virtual IComponent* GetComponentOfType(String typeName, unsigned int entityId = 0) = 0;
    
    virtual void GetAllComponents(std::vector<IComponent*> &components) = 0;
    virtual bool GetComponentIteratorOfType(String typeName, StringMap::iterator &iterator) = 0;

    template<class T>
    T* GetComponentOfType(unsigned int entityId = 0)
    {
