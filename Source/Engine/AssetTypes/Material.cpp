...
void FindUniformIndex(int &index, std::vector<String> &names,
                      const String &name) {
...
void Material::GetFloatUniforms(std::vector<std::string> &names,
...
void Material::GetVectorUniforms(std::vector<std::string> &names,
  ...
  for (size_t x = 0; x < VectorUniformNames.size(); x++) {
    ...
    if (y >= VectorUniformValues.size())
      throw 1;
    values.push_back(
        glm::vec4(VectorUniformValues[y + 0], VectorUniformValues[y + 1],
