...
unsigned int OglTextureManager::GetTextureIdByAssetId(unsigned int textureAssetId)
{
...
void OglTextureManager::LoadTextureToGpu(unsigned int textureAssetId, unsigned int &texture)
{
    int width, height;
    Texture2D* textureAsset = Statics::Get<IAssetManager>()->GetAssetOfType<Texture2D>(textureAssetId);
...
void OglTextureManager::GetTextureSlotForShaderProgramByName(const std::string &name, int programId, 
    int &textureSlot, bool &isNew)
{
    isNew = false;
