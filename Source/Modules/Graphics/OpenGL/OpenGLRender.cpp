...
bool OpenGLRender::WindowShouldClose() { return glfwWindowShouldClose(Window); }

void OpenGLRender::Create(unsigned short width, unsigned short height,
                          const String &title) {
  ...
  Window = glfwCreateWindow(FrameWidth, FrameHeight, title.GetCharArray(), NULL,
                            NULL);
  if (!Window) {
    glfwTerminate();
    exit(EXIT_FAILURE);
...
float OpenGLRender::GetFrameAspectRatio() { return FrameAspectRatio; }
...
VaoMeshManager *OpenGLRender::GetMeshManager() { return MeshManager; }

OglShaderManager *OpenGLRender::GetShaderManager() { return ShaderManager; }

OglTextureManager *OpenGLRender::GetTextureManager() { return TextureManager; }

bool OpenGLRender::IsWindowCreated() { return Window != nullptr; }
