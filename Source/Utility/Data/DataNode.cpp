...
unsigned int DataNode::GetUniqueID()
{
    return UniqueID;
}

void DataNode::SetUniqueID(const unsigned int &newID)
{
    UniqueID = newID;
}
...
ISerialized* DataNode::Deserialize()
{
    ...
    for (size_t x = 0; x < Attributes.size(); x++)
    {
        ...
        if (Attributes[x]->TypeName() == "SerializedClass")
        {
            ...
            TypedAttribute<IDataNode*>* attr = dynamic_cast<TypedAttribute<IDataNode*>*>(Attributes[x]);
            // d
...
ISerialized* DataNode::MakeTypedAttribute(SSD::Attribute* attribute)
{
    ...
    for (unsigned int x = 0; x < attribute->ElementCount * stride; x+=stride)
    {
        ...
        if (typeName == "int") 
        {
            if (attribute->IsSingleElement)
            {
                ISerialized* singleElementAttribute = new TypedAttributeValue<int>(name, typeName, DataStruct::GetInt32(value));
                delete[] value;
        ...
        if (typeName == "unsigned int" || typeName == "uid") 
        {
            if (attribute->IsSingleElement)
            {
                ISerialized* singleElementAttribute = new TypedAttributeValue<unsigned int>(name, typeName, DataStruct::GetUInt32(value));
                delete[] value;
                return singleElementAttribute;
            }   
        ...
        if (typeName == "unsigned short") 
        {
            if (attribute->IsSingleElement)
            {
                ISerialized* singleElementAttribute = new TypedAttributeValue<unsigned short>(name, typeName, DataStruct::GetUShort(value));
                delete[] value;
                return singleElementAttribute;
            }   
        ...
        if (typeName == "short") 
        {
            if (attribute->IsSingleElement)
            {
                ISerialized* singleElementAttribute = new TypedAttributeValue<short>(name, typeName, DataStruct::GetShort(value));
                delete[] value;
                return singleElementAttribute;
            }
        ...
        if (typeName == "float") 
        {
            if (attribute->IsSingleElement)
            {
                ISerialized* singleElementAttribute = new TypedAttributeValue<float>(name, typeName, DataStruct::GetFloat(value));
                delete[] value;
                return singleElementAttribute;
            }
        ...
        if (typeName == "unsigned char") 
        {
            if (attribute->IsSingleElement)
            {
                ISerialized* singleElementAttribute = new TypedAttributeValue<unsigned char>(name, typeName, value[0]);
                delete[] value;
                return singleElementAttribute;
            }
